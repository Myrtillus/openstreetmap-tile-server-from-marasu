...
build: 
	docker build -t registry.finomena.fi/c/openstreetmap-tile-server:0.4.4 --build-arg NOCACHE=$$(date +%s) .

push: build
	docker push registry.finomena.fi/c/openstreetmap-tile-server:0.4.4
